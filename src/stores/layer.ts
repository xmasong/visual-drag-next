...
export const useLayer = defineStore("layer", () => {
  ...
  function upComponent() {
    const store = useComponent();
    if (typeof store.curComponentIndex !== "number") return;
  ...
  function topComponent() {
    const store = useComponent();
    if (typeof store.curComponentIndex !== "number" || !store.curComponent)
      return;
    if (store.curComponentIndex < store.componentData.length - 1) {
      store.componentData.splice(store.curComponentIndex, 1);
      store.componentData.push(store.curComponent);
      store.curComponentIndex = store.componentData.length - 1;
    } else {
      ElMessage.error("已经到顶了");
    }
  }
  ...
  function bottomComponent() {
    ...
    if (store.curComponentIndex > 0) {
      store.componentData.splice(store.curComponentIndex, 1);
      store.componentData.unshift(store.curComponent);
      store.curComponentIndex = 0;
    } else {
      ElMessage.error("已经到底了");
    }
  }
